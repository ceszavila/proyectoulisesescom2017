...
\begin{UseCase}{CUPR 2}{Modificar residuo sólido del plan de acción}
    {
	...
	Este caso de uso permite al actor modificar un residuo sólido y la cantidad de este que se desea disminuir o reciclar.
    }
...
% 			% PC: Petición de Cambio, describa el trabajo a realizar, si es posible indique la causa de la PC. Opcionalmente especifique la fecha en que considera razonable que se deba terminar la PC. No olvide que la numeración no se debe reiniciar en una segunda o tercera revisión.
 			\RCitem{PC1}{\DONE{Resumen: cuando se utiliza este caso de uso?}}{Fecha de entrega}
 			\RCitem{PC2}{\DONE{Entradas: Liga rota en Tipo, residuo sólido, Total semanal}}{Fecha de entrega}
 			\RCitem{PC2}{\DONE{Entradas: Liga rota en Tipo, residuo sólido, Total semanal}}{Fecha de entrega} 						\RCitem{PC4}{\TOCHK{Precondiciones: Liga rota en tipo de residuo}}{Fecha de entrega} 			
 			\RCitem{PC5}{\TOCHK{Precondiciones: En liga de residuo sólido, en la entidad sólo se llama residuo}}{Fecha de entrega} 			
			\RCitem{PC2}{\DONE{Salidas: Liga rota en Tipo, residuo sólido, Total semanal}}{Fecha de entrega} 			
 			\RCitem{PC2}{\DONE{Trayectoria alternativa C: es Fin de caso de uso, no de trayectoria}}{Fecha de entrega}
...
% %--------------------------------------------------------
    ...
    \UCitem{Propósito}{Modificar la información referente a un residuo sólido y a la cantidad que se desea disminuir o reciclar de este.}
    \UCitem{Entradas}{
	\begin{UClist}
	   \UCli {\bf Información del residuo sólido:}
	    \begin{itemize}
	      \item \cdtRef{gls:tipoDeResiduo}{Tipo}: \ioSeleccionar.
	      \item \cdtRef{gls:residuo}{Residuo}: \ioSeleccionar.
    ...
    \UCitem{Salidas}{
	\begin{UClist}
	    \UCli Información del residuo sólido
	    \begin{itemize}
	      \item \cdtRef{gls:tipoDeResiduo}{Tipo}: \ioObtener.
	      \item \cdtRef{gls:residuo}{Residuo}: \ioObtener.
	      \item \cdtRef{residuoSolidoMeta:cantidad}{Total semanal (kg/semana)}: \ioObtener.
	    \end{itemize}
	\end{UClist}
    }
...
\UCli {\bf Interna:} Que la escuela se encuentre en estado \cdtRef{estado:planEdicion}{Plan de acción en edición}.
	  ...
	  \UCli Que el \cdtRef{gls:enfoqueMeta}{enfoque de la meta} que se está modificando sea ``Reducción o reciclaje de residuos''.	
	\end{UClist}
    }
    
    \UCitem{Postcondiciones}{
	\begin{UClist}
	    \UCli {\bf Interna:} Se modificará el residuo sólido del plan de acción en el sistema.
	\end{UClist}
    }
	...
	\begin{UClist}
	    ...
	    \UCli \cdtIdRef{MSG5}{Falta un dato requerido para efectuar la operación solicitada}: Se muestra en la pantalla \cdtIdRef{IUPR 2}{Modificar residuo sólido del plan de acción} cuando no se ha ingresado un dato marcado como requerido.	    
	    \UCli \cdtIdRef{MSG6}{Formato incorrecto}: Se muestra en la pantalla \cdtIdRef{IUPR 2}{Modificar residuo sólido del plan de acción} cuando el tipo de dato ingresado no cumple con el tipo de dato solicitado en el campo.
	    \UCli \cdtIdRef{MSG7}{Se ha excedido la longitud máxima del campo}: Se muestra en la pantalla \cdtIdRef{IUPR 2}{Modificar residuo sólido del plan de acción} cuando se ha excedido la longitud de alguno de los campos.
...
\end{UseCase}
 ...
 \begin{UCtrayectoria}
    \UCpaso[\UCactor] Solicita modificar un residuo sólido del plan de acción oprimiendo el botón \botEdit del registro que desea modificar en la pantalla \cdtIdRef{IUPRS 1}{Registrar meta de residuos sólidos} o en la pantalla \cdtIdRef{IUPRS 2}{Modificar meta de residuos sólidos}.
    ...
    \UCpaso[\UCsist] Muestra la pantalla \cdtIdRef{IUPR 2}{Modificar residuo sólido del plan de acción}.
    \UCpaso[\UCactor] Ingresa los nuevos datos del residuo sólido del plan de acción en la pantalla \cdtIdRef{IUPR 2}{Modificar residuo sólido del plan de acción}. \label{cupr2:ingresarInfo}
    \UCpaso[\UCactor] Solicita la modificación del residuo sólido del plan de acción oprimiendo el botón \cdtButton{Aceptar} en la pantalla \cdtIdRef{IUPR 2}{Modificar residuo sólido del plan de acción}. \refTray{D}.
	\UCpaso[\UCsist] Verifica que la escuela se encuentre en  estado ``Plan de acción en edición''. \refTray{A}.    
    ...
    \UCpaso[\UCsist] Muestra la pantalla \cdtIdRef{IUPRS 1}{Registrar meta de residuos sólidos} o la pantalla \cdtIdRef{IUPRS 2}{Modificar meta de residuos sólidos} con la nueva información del residuo sólido del plan de acción. 
...
\begin{UCtrayectoriaA}[Fin del caso de uso]{A}{La escuela no se encuentra en el estado ``Plan de acción en edición''.}
 ...
 \end{UCtrayectoriaA}
    ...
    \UCpaso[\UCactor] Solicita cancelar la operación oprimiendo el botón \cdtButton{Cancelar} en la pantalla \cdtIdRef{IUPR 2}{Modificar residuo sólido del plan de acción}.
    ...
    \UCpaso[\UCsist] Muestra el mensaje \cdtIdRef{MSG5}{Falta un dato requerido para efectuar la operación solicitada} en la pantalla \cdtIdRef{IUPR 2}{Modificar residuo sólido del plan de acción} indicando que la modificación del residuo sólido no puede realizarse debido a la falta de información requerida.
    ...
    \UCpaso[\UCsist] Muestra el mensaje \cdtIdRef{MSG6}{Formato incorrecto} en la pantalla \cdtIdRef{IUPR 2}{Modificar residuo sólido del plan de acción} indicando que la modificación del residuo sólido no puede realizarse debido a que la información ingresada no es correcta.
    \UCpaso[] Continúa con el paso \ref{cupr2:ingresarInfo} de la trayectoria principal.     
    \end{UCtrayectoriaA}
    
            \begin{UCtrayectoriaA}{G}{El actor ingresó un dato que excede la longitud máxima.}    
    \UCpaso[\UCsist] Muestra el mensaje \cdtIdRef{MSG7}{Se ha excedido la longitud máxima del campo} en la pantalla \cdtIdRef{IUPR 2}{Modificar residuo sólido del plan de acción} indicando que la modificación del residuo sólido no puede realizarse debido a que la longitud del campo excede la longitud máxima definida.
    \UCpaso[] Continúa con el paso \ref{cupr2:ingresarInfo} de la trayectoria principal.     
    \end{UCtrayectoriaA}
